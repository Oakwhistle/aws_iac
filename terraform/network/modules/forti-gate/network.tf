...
resource "aws_internet_gateway" "fgtvmigw" {
  ...
  tags = merge(
    {
      Name = "${var.name_prefix}-FTGVM-IGW"
    },
  var.tags)
}
...
resource "aws_route_table" "fgtvmpublicrt" {
  ...
  tags = merge(
    {
      Name = "${var.name_prefix}-FTGVM-PUBLIC-RT"
    },
  var.tags)
}
...
resource "aws_route_table" "fgtvmprivatert" {
  ...
  tags = merge(
    {
      Name = "${var.name_prefix}-FTGVM-PRIVATE-RT"
    },
  var.tags)
}
...
resource "aws_security_group" "public_allow" {
  ...
  tags = merge(
    {
      Name = "${var.name_prefix}-FTGVM-PUBLIC-SG"
    },
  var.tags)
}
...
resource "aws_security_group" "allow_all" {
  ...
  tags = merge(
    {
      Name = "${var.name_prefix}-FTGVM-ALLOW-ALL-SG"
    },
  var.tags)
}
